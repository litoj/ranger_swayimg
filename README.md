...
- The window hides only once the preview script reports "not an image", a folder is selected, or the last entry of a folder is deleted (an empty selection emits no preview signal) — slow raw thumbnails never flicker through a premature hide.
